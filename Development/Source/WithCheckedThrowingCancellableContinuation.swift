...
/// It also supports cooperative task cancelation, so if the given closure returns a cancellable operation, it will be cancelled once the current task is cancelled.
///
/// - Parameters:
...
/// - Throws: An error passed to a completion by the given closure, or ``CancellationError`` if the current task was cancelled before the given closure called a completion.
/// - Returns: A result passed to a completion by the given closure.
public func withCheckedThrowingCancellableContinuation<T>(function: String = #function, _ body: (@escaping (Result<T, Error>) -> Void) -> TCCancellable?) async throws -> T {

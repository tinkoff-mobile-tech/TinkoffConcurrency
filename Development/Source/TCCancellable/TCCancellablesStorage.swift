...
public final class TCCancellablesStorage: TCCancellable {
    ...
    public enum State {
        ...
        ///
        /// Calls of methods ``TCCancellablesStorage/cancel()`` and ``TCCancellablesStorage/deactivate()`` will have effect.
        case active

        /// A cancelled state indicating that all added cancellable operations were cancelled.
        ///
        /// In this state, not only a cancellable operation can **not** be added using ``TCCancellablesStorage/add(_:)`` method, but it also will be **cancelled**.
        ///
        /// Calls of methods ``TCCancellablesStorage/cancel()`` and ``TCCancellablesStorage/deactivate()`` are ignored.
        case cancelled

        /// A deactivated state indicating that all added cancellable operations were discarded.
        ///
        /// In this state, a cancellable operation can **not** be added using ``TCCancellablesStorage/add(_:)`` method and will be **ignored**.
        ///
        /// Calls of methods ``TCCancellablesStorage/cancel()`` and ``TCCancellablesStorage/deactivate()`` are ignored.
        case deactivated
    }
    ...
    ///
    /// If the storage is in the ``TCCancellablesStorage/State-swift.enum/cancelled`` state, cancellable operation will **not** be added and will be **cancelled**.
    ///
    /// If the storage is in the ``TCCancellablesStorage/State-swift.enum/deactivated`` state, cancellable operation will **not** be added and will be **ignored**.
    ///
    /// - Parameters:
    ...
    /// Repeated calls of this method will be ignored.
    ///
    /// - Returns: `true` if the storage was in the ``TCCancellablesStorage/State-swift.enum/active`` state before the method was called; otherwise returns `false`.
